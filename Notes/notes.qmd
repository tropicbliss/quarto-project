...
-   `sf` functions
    ...
    -   `st_intersects`: whether two geometries touch or overlap each other
        ...
        -   Note that many `sf` functions may sound similar to each other but perform totally different tasks. For instance, `st_intersection` computes the intersection of two geometries but `st_intersects` merely tells you whether it intersects each other or not.
