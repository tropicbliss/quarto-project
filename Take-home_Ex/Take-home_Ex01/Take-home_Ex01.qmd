...
for(i in tims){
  ...
  plot(st_geometry(townships), add=TRUE) # overlap the polygons of townships on top
  text(st_coordinates(st_centroid(townships)), labels=townships$TS, cex=0.5, col="white") # add text at the centroids of each polygon
}
```
...
railways <- st_read("data", layer = "mmr_rlwl_250k_mimu_1") %>% st_transform(crs = 32647) %>% st_cast("LINESTRING") %>% filter(Div_Name == "Yangon")
railways <- st_intersection(railways, yangon_bounds) %>% st_cast("LINESTRING")
rivers <- st_read("data", layer = "myanmar_river_network_250k") %>% st_transform(crs = 32647) %>% st_cast("LINESTRING")
...
```{r}
roads <- st_read("data", layer = "mmr_rdsl_mimu_250k") %>% st_transform(crs = 32647) %>% st_cast("LINESTRING")
```
